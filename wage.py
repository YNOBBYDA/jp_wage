...
st.header('■２０１９−一人当たり平均賃金HeatMap')
...
# 正規化処理を行う。
xxx = df_pref_map['一人当たり賃金（万円）'] 
df_pref_map['一人当たり賃金（相対値）'] = (xxx - xxx.min())/(xxx.max() - xxx.min())

#df_pref_map

#ヒートマップ
view = pdk.ViewState(
    longitude=139.691684,
    latitude=35.689185,
    zoom=4,
    pitch=40.5
)
...
max_x = df_mean_categ[option_wage].max() * 1.1
...
st.plotly_chart(fig)
st.write("出典：RESAS（地域経済分析システム）")
st.write("本結果はRESAS（地域経済分析システム）を加工して作成")